...
export const imsByCustomerSummary = (props) => {
    ...
    useEffect(() => {
        ...
        tableService.IMSBYCUSTSUMADMIN(from, to,supplierId,locationId).then((res) => {
            if (res.data.length != 0) {
                // res.data.forEach(element => {
                //     dataArray.push(
                //         Object.keys(element).map(val => element[val])
...
export const imsByCustomerChannel = (props) => {
    ...
    useEffect(() => {
        ...
        tableService.IMSBYCUSTCHANNELADMIN(from, to,supplierId,locationId).then((res) => {
            if (res.data.length != 0) {
                // res.data.forEach(element => {
                //     dataArray.push(
                //         Object.keys(element).map(val => element[val])
...
export const imsBySupplier = (props) => {
    ...
    useEffect(() => {
        ...
        tableService.IMSSupplier(from, to,bySupplierPrice,supplierId).then((res) => {
            if (res.data.length != 0) {
                // res.data.forEach(element => {
                //     dataArray.push(
                //         Object.keys(element).map(val => element[val])
